...
export default async function AdminModerationPage({ searchParams }: Props) {
  ...
  const duplicateRefundLedgerTxCount = canUseDb
    ? await db.walletTransaction
      .groupBy({
        by: ["relatedBetId"],
        where: {
  ...
  const integrityRiskScore = Math.min(100, (settledWithNullPayoutCount * 40) + (unresolvedSettledBacklogCount * 25) + (resolvedWithoutResolutionCount * 15) + (nonPositiveBetAmountCount * 30) + (missingSettlementLedgerTxCount * 32) + (duplicateSettlementLedgerTxCount * 22) + (duplicateRefundLedgerTxCount * 24) + (orphanTargetReportCount * 8) + (hasPayoutRatioOutlier ? 20 : 0));
  ...
  const settlementRecoveryQueue = [
    ...
    {
      id: "recovery-refund-dup",
      label: "환불 원장 중복 정리",
      count: duplicateRefundLedgerTxCount,
      detail: "동일 베팅 BET_REFUND 중복 기록",
      href: "/admin/topics?status=CANCELED",
      tone: duplicateRefundLedgerTxCount > 0 ? "danger" : "ok",
      cta: "환불 원장 정리",
    },
  ] as const;
  ...
  const integrityWatchItems = [
    ...
    {
      key: "duplicate-refund-ledger-transaction",
      label: "환불 원장 중복",
      count: duplicateRefundLedgerTxCount,
      description: "동일 베팅에 BET_REFUND 트랜잭션이 2건 이상 기록된 건",
      href: "/admin/topics?status=CANCELED",
      actionLabel: "환불 중복 원장 정리",
      tone: duplicateRefundLedgerTxCount > 0 ? "danger" : "ok",
    },
    {
  ...
  const settlementGuardrails = [
    ...
    {
      key: "refund-ledger-duplicate",
      label: "환불 원장 중복 없음",
      count: duplicateRefundLedgerTxCount,
      helper: "동일 relatedBetId BET_REFUND 중복 여부",
      tone: duplicateRefundLedgerTxCount > 0 ? "danger" : "ok",
    },
    {
  ...
  const mobilePolishSignals = [
    {
      ...
      value: urgentReportCount > 0 ? "Escalated" : "Calm",
      hint: urgentReportCount > 0 ? `OPEN ${urgentReportCount}건이 상단 우선순위` : "핵심 카드 기준 상단-하단 리듬 유지",
      tone: urgentReportCount > 0 ? "danger" : "ok",
    },
    {
      id: "spacing",
      ...
      value: "8pt cadence",
      hint: "카드 간격 · 타이포 스텝 · CTA 높이를 모바일 기준으로 정렬",
      tone: "neutral",
    },
    {
      id: "thumb",
      label: "Thumb reach",
      value: "Primary ready",
      hint: nextPriorityReport ? "다음 처리 카드까지 한 번 스크롤 동선" : "즉시 처리 항목 없음",
      tone: nextPriorityReport ? "warning" : "ok",
    },
    {
      id: "states",
      label: "State clarity",
      value: hasCriticalIntegrityIssue ? "Alert" : "Stable",
      hint: hasCriticalIntegrityIssue ? "무결성 경고 카드와 복구 액션 노출" : "Empty/Error/Stable 패턴 정돈",
      tone: hasCriticalIntegrityIssue ? "danger" : "ok",
    },
  ] as const;
  ...
  return (
    <PageContainer>
      <section className="admin-hero-shell">
        ...
        <div className="admin-pulse-grid" style={{ marginTop: "0.75rem" }}>
          ...
          <div className="admin-pulse-card">
            ...
            <span className="admin-kpi-meta">{integritySeverityLabel} · 누락 · 백로그 · 불일치 · 비정상 금액 · 정산/환불 원장 누락·중복 · 배당률 · 신고 대상 유실</span>
          </div>
          <div className="admin-pulse-card">
      ...
      <Card className="admin-polish-strip-card" aria-label="모바일 제품 완성도 신호">
        <div className="admin-polish-strip-head">
          <p className="admin-jump-nav-label">Product polish benchmark</p>
          <Pill tone={hasCriticalIntegrityIssue ? "danger" : "success"}>{hasCriticalIntegrityIssue ? "Needs attention" : "On track"}</Pill>
        </div>
      ...
      <Card className="admin-integrity-command-bar" aria-label="신고/정산 무결성 즉시 명령">
        <div className="admin-integrity-command-head">
          ...
          <Pill tone={integrityIssueTotal > 0 ? "danger" : "success"}>{integrityIssueTotal > 0 ? `복구 ${integrityIssueTotal}건` : "무결성 정상"}</Pill>
        </div>
        <div className="admin-integrity-command-grid">
          ...
          <Link href="/admin/topics?status=RESOLVED" className={`admin-integrity-command-item is-${resolvedWithoutResolutionCount > 0 ? "warning" : "ok"}`}>
            ...
            <small>resolution 누락</small>
          </Link>
          <Link href="#integrity-watch" className={`admin-integrity-command-item is-${hasPayoutRatioOutlier ? "warning" : "ok"}`}>
            <span>배당률 밴드</span>
          ...
          <Link href="/admin/topics?status=CANCELED" className={`admin-integrity-command-item is-${duplicateRefundLedgerTxCount > 0 ? "danger" : "ok"}`}>
            <span>환불 중복</span>
            <strong>{duplicateRefundLedgerTxCount}건</strong>
            <small>BET_REFUND 중복 기록</small>
          </Link>
        </div>
      </Card>
