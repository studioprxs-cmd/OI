...
export function FeedCard({ title, description, meta, badge, footer, children, thumbnailSrc, thumbnailAlt, featuredThumbnail }: FeedCardProps) {
  const thumbnailClass = thumbnailSrc ? `has-thumbnail ${featuredThumbnail ? "is-featured" : "is-side"}` : "";

  return (
    <article className={`feed-card ${thumbnailClass}`.trim()}>
      {thumbnailSrc ? (
        <div className={`feed-card-thumbnail ${featuredThumbnail ? "is-featured" : "is-side"}`}>
